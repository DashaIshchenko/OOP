...
void menu(const int c, map<double, Pair>& m) {
	...
	case 1: {
		...
		if (op == '<') cout << m << endl;
			...
			vector<Pair> COPY;
			/*auto cmp = [](const pair<double, Pair>& p1, const pair<double, Pair>& p2) {
	...
	case 2: {
		...
		double value;
		cin >> value;
		//auto it = find_if(m.begin(), m.end(), bind2nd(ptr_fun(EQUAL), value));
		const auto it = find_if(m.begin(), m.end(), [&value](decltype(*m.begin())& it)->bool {
			return it.second == value; // comparing with the object
			});
	...
	case 4: {
		...
		//for (auto it = m.begin(); it != m.end(); it++) cout << it->second.sum() << " ";
		cout << "\nInput first border:\n"; cin >> one;
		cout << "\nInput second border:\n"; cin >> two;
