...
void menu(const int c, priority_queue<Pair>& q) {
	...
	case 3: {
		...
		sum.set_first(round(sum.get_first() / COPY.size()*100)/100); //округление до двух знаков
	...
	case 4: {
		...
		//list<Pair>::iterator
		auto s = remove_if(COPY.begin(), COPY.end(), [one, two](Pair& p) {return p.sum() > one && p.sum() < two; });
...
int main() {
	...
	//generate(l.front(), l.back(), []() {Pair p; p.set_random(); return p; });
	cout << q << endl;
	while (true) {
		system("pause");
		system("cls");
